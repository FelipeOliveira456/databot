...
def get_plot_decision_agent() -> RunnableSequence:
    ...
    8. Se o usuário pediu um gráfico específico, esse gráfico deve ser feito, independentemente das outras regras. 
...
def get_pdf_report_agent() -> RunnableSequence:
    ...
    - Um campo `error` que pode conter erros de geração anteriores (ou pode estar vazio).  
      Se houver erros listados, **não repita esses erros no relatório atual**.  
      Se não houver erros, continue normalmente.
    ...
    3. **Análise dos Gráficos**
       - Para cada gráfico:
           - Você deve colocar as imagens dessa forma: ![descrição do gráfico](nome_do_arquivo.png)
